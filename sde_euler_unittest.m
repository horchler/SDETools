...
%SDE_EULER_UNITTEST  Perform suite of unit tests on SDE_EULER solver function.
...
msg{i} = 'SDELab:sde_euler:NotEnoughInput';
...
msg{i} = 'SDELab:sdearguments:GFUNNot2DArray';
...
msg{i} = 'SDELab:sdearguments:GFUNNotColumnVector';
...
if ~isempty(ts)
    fprintf(1,sp(ones(1,ceil(log10(length(ts)+1))+39)));
end
for i=ts
    errv = true;
    try 
    ...
    num = num+1;
    if num > 1
        fprintf(1,[bs(mod(0:2*ceil(log10(num+1))-1+78,2)+1) '%d'],num);
        fprintf(1,' error tests performed successfully...\n');
    end
end
if num == 1
    pause(0.1);
    fprintf(1,[bs(mod(0:1+78,2)+1) '%d'],num);
    pause(0.1);
    fprintf(1,' error test performed successfully.\n');
elseif num > 1
    pause(0.01);
    fprintf(1,[bs(mod(0:2*ceil(log10(num+1))-1+78,2)+1) 'All %d'],num);
    fprintf(1,' error tests performed successfully.\n');
end
...
if ~isempty(ts) || ~runtests
    fprintf(1,sp(ones(1,ceil(log10(mm))+38)));
    ms = 1:m;
else
    ms = [];
end
for j=ms
    ...
    if ~isempty(tts)
        ...
        if twoout{j}(2) == 1    % output and check Wiener increments
            ...
            % output of W is same dimension as output of GFUN
            st = dbstack;
            i = i+1;
            lnum2{i} = st.line;
            cmd{i} = ['[y,w]=sde_euler(' f{j} ',' g{j} ',' tspan{j} ',' y0{j} opts{j} params{j} ');'];
        ...
        for i=tts
            ...
            num = num+1;
            if mod(num,10) == 0
                fprintf(1,[bs(mod(0:2*ceil(log10(num+1))-1+76,2)+1) '%d'],num);
...
if num == 0
    if~isempty(ms)
        fprintf(1,bs(mod(0:1+76,2)+1));
    end
elseif num == 1
    fprintf(1,[bs(mod(0:1+76,2)+1) '%d'],num);
    fprintf(1,' unit test performed successfully.\n');
else
    pause(0.01);
    fprintf(1,[bs(mod(0:2*ceil(log10(num+1))-1+76,2)+1) 'All %d'],num);
    fprintf(1,' unit tests performed successfully.\n');
end
