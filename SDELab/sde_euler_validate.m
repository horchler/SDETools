...
if nargin < 2
    error('SDELab:sde_euler_validate:NotEnoughInputs',...
          'Not enough input arguments.');
else
    if nargin < 5
        if nargin == 3
            error('SDELab:sde_euler_validate:InvalidInputPattern',...
    ...
    elseif nargin == 5
        if isempty(options) && (ndims(options) ~= 2 || ...
            ...
            error('SDELab:sde_euler_validate:InvalidSDESETStruct',...
                  'Invalid SDE options structure.  See SDESET.');
        end
    else
        error('SDELab:sde_euler_validate:TooManyInputs',...
              'Too many input arguments.');
    end
end
if nargout > 2
    error('SDELab:sde_euler_validate:TooManyOutputs',...
          'Too many output arguments.');
end

% Check DT
if ~isvector(dt) || ~isfloat(dt) || ~isreal(dt) || ~all(isfinite(dt))
    error(  'SDELab:sde_euler_validate:InvalidDT',...
            'DT must be a finite real floating-point vector.');
end
ldt = length(dt);
if length(dt) < 2
    error(  'SDELab:sde_euler_validate:BadInputSizeDT',...
...
if ~isscalar(n) || ~isfloat(n) || ~isreal(n) || ~isfinite(n)
    error(  'SDELab:sde_euler_validate:InvalidN',...
            'N must be a finite real floating-point scalar.');
end
if isempty(n) || n < 1 || n-floor(n) ~= 0
    error(  'SDELab:sde_euler_validate:BadInputSizeN',...
...
if ~isempty(RandSeed)
    if ~isscalar(RandSeed) || ~isnumeric(RandSeed) || ~isreal(RandSeed) || ...
        ...
        error('SDELab:sde_euler_validate:InvalidRandSeed',...
