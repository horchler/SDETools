...
if nargin < 6
    if nargin < 5
        error(  'SDELab:sde_ou:NotEnoughInputs',...
                'Not enough input arguments.  See %s.',func);
    end
    if isa(y0,'struct')
        error(  'SDELab:sde_ou:NotEnoughInputsOptions',...
...
if ~all(strcmp(dataType,{class(th),class(mu),class(sig),class(tspan),...
        class(y0)}))
    warning( 'SDELab:sde_ou:InconsistentDataType',...
    ...
    if CustomRandFUN    % check output of alternative RandFUN
        try
            ...
            if ndims(r) ~= 2 || isempty(r) || ~isfloat(r)
                error(  'SDELab:sde_ou:RandFUNNot2DArray3',...
                       ['RandFUN must return a non-empty matrix of floating '...
                        'point values.  See %s.'],solver);
            end
            [m n] = size(r);
            if m ~= lt-1 || n ~= N
                error(  'SDELab:sde_ou:RandFUNDimensionMismatch3',...
